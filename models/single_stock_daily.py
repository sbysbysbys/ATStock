...
if __name__ == '__main__':
        ...
        if use_cuda:
            if dist.get_rank() == 0:
                for i, arg in enumerate(args):
                    print(arg, end=" " if i < len(args) - 1 else "\n")
        else:
            for i, arg in enumerate(args):
                print(arg, end=" " if i < len(args) - 1 else "\n")
    ...
    check_print("start data preparation")
    ...
    if train_before_test:
        train_dataset, test_dataset = get_train_and_test_loader(s_data)
    else:
        train_dataset, test_dataset = get_train_and_test_loader2(s_data)
    ...
    if use_cuda:
        ...
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank], output_device=local_rank)
        world_size = dist.get_world_size()
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)    # syncBN
    
    end_data_preparation = time.time()
    check_print("data preparation time = ", end_data_preparation - start_data_preparation)

    if args.train:
        check_print("start training")
        ...
        for i in range(num_epochs+num_epochs_decay):
            ...
            # 打乱顺序
            if use_cuda:
                check_print("use_cuda::set_epoch")
            ...
            end_time = time.time()
            check_print("epoch = ", i+1, "trend_loss = ", avg_trend_loss, "mse_loss = ", avg_mse_loss, "correctness = ", avg_correctness, "time = ", end_time - start_time)

            if (use_cuda == True and dist.get_rank() == 1) or use_cuda == False:
...
#         model.to(device)
#         model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)           # sync batchnorm 
